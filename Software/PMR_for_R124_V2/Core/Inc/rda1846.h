...
#define RDA1846_PAGE_UP    0x0001
#define RDA1846_PAGE_DOWN  0x0000
#define RDA1846_ADDR_LIMIT 0x80

// Využití unionu a bitfields pro bezpečnější přístup k registru 0x30
typedef union {
    uint16_t value;
    struct {
        uint16_t soft_reset   : 1;  // Bit 0
        uint16_t chip_cal_en  : 1;  // Bit 1
        uint16_t pdn_reg      : 1;  // Bit 2
        uint16_t sq_on        : 1;  // Bit 3
        uint16_t vox_on       : 1;  // Bit 4
        uint16_t rx_on        : 1;  // Bit 5
        uint16_t tx_on        : 1;  // Bit 6
        uint16_t mute         : 1;  // Bit 7
        uint16_t st_mode      : 2;  // Bity 9:8
        uint16_t reserved     : 1;  // Bit 10
        uint16_t tail_elim_en : 1;  // Bit 11
        uint16_t channel_mode : 2;  // Bity 13:12
        uint16_t others       : 2;  // Bity 15:14
    } bits;
} RDA1846_Reg30_t;
